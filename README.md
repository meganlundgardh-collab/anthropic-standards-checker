...
(These counts predate a later fix to `LINK_RE`/`normalize()` that made fragment links — `/path#section` — resolvable at all; see `output/run_log.md`'s "What changed in this rerun" for the current 262-link, 109-ANCHOR_MISMATCH numbers, and for a second, distinct false-positive pattern that fix surfaced. The Jaccard-vs-overlap-coefficient decision recorded here is still correct and still the reason for that choice; only the sample size changed.)
